    ...
    protected virtual void UpdateMousePos(ImGuiIOPtr io)
    {
        ...
        if (io.ConfigFlags.HasFlag(ImGuiConfigFlags.ViewportsEnable))
        {
            if (io.WantSetMousePos)
            {
                ...
                foreach (var w in DisplayServer.GetWindowList())
                    if (DisplayServer.WindowIsFocused(w))
                    {
                        var winPos = DisplayServer.WindowGetPosition(w);
                        Godot.Input
                            .WarpMouse(new Vector2(io.MousePos.X - winPos.X, io.MousePos.Y - winPos.Y));
                        break;
                    }
            }
            else
            {
                ...
                uint viewportID = 0;
                var windowID = DisplayServer.GetWindowAtScreenPosition(mousePos);
                if (windowID != -1)
                    unsafe
                    {
                        var vp = ImGui.FindViewportByPlatformHandle(windowID);
                        if (vp.NativePtr != null) viewportID = vp.ID;
                    }
...
#pragma warning restore IDE0004 // Remove Unnecessary Cast
    ...
    protected bool HandleEvent(InputEvent evt)
    {
        ...
        {
            UpdateKeyMods(io);
            var igk = ConvertKey(k.Keycode);
            var pressed = k.Pressed;
            var unicode = k.Unicode;

            if (igk != ImGuiKey.None) io.AddKeyEvent(igk, pressed);

            if (pressed && unicode != 0 && io.WantTextInput) io.AddInputCharacterUTF16((ushort)unicode);
        ...
        else if (io.ConfigFlags.HasFlag(ImGuiConfigFlags.NavEnableGamepad))
        {
            ...
            {
                var igk = ConvertJoyButton(jb.ButtonIndex);
