...
#pragma warning restore CA1819 // 属性不应返回数组
    ...
    {
        if (WriterIndex + size > Capacity) EnsureWrite0(size);
    }

    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    private void EnsureRead(int size)
    {
        if (ReaderIndex + size > WriterIndex) throw new SerializationException();
    }
    ...
    public uint ReadUint()
    {
        ...
        {
            EnsureRead(4);
            var x = ((h & 0x0f) << 24) | ((uint)Bytes[ReaderIndex + 1] << 16) | ((uint)Bytes[ReaderIndex + 2] << 8) |
                    Bytes[ReaderIndex + 3];
            ReaderIndex += 4;
            return x;
        }
        ...
        {
            EnsureRead(5);
            var x = ((uint)Bytes[ReaderIndex + 1] << 24) | (uint)(Bytes[ReaderIndex + 2] << 16) |
                    ((uint)Bytes[ReaderIndex + 3] << 8) | Bytes[ReaderIndex + 4];
            ReaderIndex += 5;
            return x;
        }
    }
...
#else
        x = (Bytes[ReaderIndex + 3] << 24) | (Bytes[ReaderIndex + 2] << 16) | (Bytes[ReaderIndex + 1] << 8) |
            Bytes[ReaderIndex];
    ...
    public ulong ReadUlong()
    {
        ...
        {
            EnsureRead(5);
            var xl = ((uint)Bytes[ReaderIndex + 1] << 24) | (uint)(Bytes[ReaderIndex + 2] << 16) |
                     ((uint)Bytes[ReaderIndex + 3] << 8) | Bytes[ReaderIndex + 4];
            var xh = h & 0x07;
            ReaderIndex += 5;
        ...
        {
            EnsureRead(6);
            var xl = ((uint)Bytes[ReaderIndex + 2] << 24) | (uint)(Bytes[ReaderIndex + 3] << 16) |
                     ((uint)Bytes[ReaderIndex + 4] << 8) | Bytes[ReaderIndex + 5];
        ...
        {
            EnsureRead(7);
            var xl = ((uint)Bytes[ReaderIndex + 3] << 24) | (uint)(Bytes[ReaderIndex + 4] << 16) |
                     ((uint)Bytes[ReaderIndex + 5] << 8) | Bytes[ReaderIndex + 6];
        ...
        {
            EnsureRead(8);
            var xl = ((uint)Bytes[ReaderIndex + 4] << 24) | (uint)(Bytes[ReaderIndex + 5] << 16) |
                     ((uint)Bytes[ReaderIndex + 6] << 8) | Bytes[ReaderIndex + 7];
            var xh = /*((h & 0x01) << 24) |*/ ((uint)Bytes[ReaderIndex + 1] << 16) |
                                              ((uint)Bytes[ReaderIndex + 2] << 8) | Bytes[ReaderIndex + 3];
            ReaderIndex += 8;
            return ((ulong)xh << 32) | xl;
        }
        else
        {
            EnsureRead(9);
            var xl = ((uint)Bytes[ReaderIndex + 5] << 24) | (uint)(Bytes[ReaderIndex + 6] << 16) |
                     ((uint)Bytes[ReaderIndex + 7] << 8) | Bytes[ReaderIndex + 8];
            var xh = ((uint)Bytes[ReaderIndex + 1] << 24) | ((uint)Bytes[ReaderIndex + 2] << 16) |
                     ((uint)Bytes[ReaderIndex + 3] << 8) | Bytes[ReaderIndex + 4];
            ReaderIndex += 9;
            return ((ulong)xh << 32) | xl;
        }
...
#else
        var xl = (Bytes[ReaderIndex + 3] << 24) | (Bytes[ReaderIndex + 2] << 16) | (Bytes[ReaderIndex + 1] << 8) |
                 Bytes[ReaderIndex];
        var xh = (Bytes[ReaderIndex + 7] << 24) | (Bytes[ReaderIndex + 6] << 16) | (Bytes[ReaderIndex + 5] << 8) |
                 Bytes[ReaderIndex + 4];
    ...
    {
        var n = o.Size;
        if (n > 0) WriteBytesWithoutSize(o.Bytes, o.ReaderIndex, n);
    }
    ...
    public EDeserializeError TryDeserializeInplaceByteBuf(int maxSize, ByteBuf inplaceTempBody)
    {
        ...
        try
        {
            ...
            else if (h < 0xf0)
            {
                ...
                n = ((h & 0x0f) << 24) | (Bytes[ReaderIndex + 1] << 16) | (Bytes[ReaderIndex + 2] << 8) |
                    Bytes[ReaderIndex + 3];
                ReaderIndex += 4;
            }
            else
            ...
            if (n > maxSize) return EDeserializeError.EXCEED_SIZE;
            if (Remaining < n) return EDeserializeError.NOT_ENOUGH;
        ...
        {
            if (!commit) ReaderIndex = oldReadIndex;
        }
    ...
    public void ReadSegment(out int startIndex, out int segmentSize)
    {
        ...
        {
            EnsureRead(3);
            segmentSize = ((h & 0x0f) << 24) | ((int)Bytes[ReaderIndex] << 16) | ((int)Bytes[ReaderIndex + 1] << 8) |
                          Bytes[ReaderIndex + 2];
            var endPos = ReaderIndex + segmentSize;
            Bytes[ReaderIndex] = Bytes[endPos];
            Bytes[ReaderIndex + 1] = Bytes[endPos + 1];
        ...
        if (ReaderIndex > WriterIndex) throw new SerializationException("segment data not enough");
    }
    ...
    public override string ToString()
    {
        ...
        for (var i = ReaderIndex; i < WriterIndex; i++) datas[i - ReaderIndex] = Bytes[i].ToString("X2");
    ...
    public static ByteBuf FromString(string value)
    {
        ...
        for (var i = 0; i < data.Length; i++) data[i] = byte.Parse(ss[i]);
    ...
    {
        var hash = 17;
        for (var i = ReaderIndex; i < WriterIndex; i++) hash = hash * 23 + Bytes[i];
        return hash;
    }
